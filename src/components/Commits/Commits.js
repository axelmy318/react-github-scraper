...
const Commits = ({ label, maxDisplayed, commitCallback }) => {
    ...
    return (<div className='github-scraper-component component-commits'>
        {content[CONTENT_KEY] !== null && content[CONTENT_KEY].success && <>
            ...
            {maxDisplayed > 0 && <div className='github-scraper-component-content'>{content[CONTENT_KEY].data.slice(0, maxDisplayed).map((commit, index, array) => commitCallback(commit, index, array))}</div>}
            {maxDisplayed <= 0 && <div className='github-scraper-component-content'>{content[CONTENT_KEY].data.map((commit, index, array) => commitCallback(commit, index, array))}</div>}
        </>}
    </div>)
}
